...
@section('breadcumb', 'Forum / Create')
@section('back', route('forum.index'))

@section('content')
    <div class="row judul">
        <form action="{{route('forum.createproses')}}" id="form"  method="post" enctype="multipart/form-data" class="w-100">
            @csrf
            <div class="col-md-12 px-0 header-detail">
                <div class="row px-2">
                    ...
                    <div class="col-md-10 col-sm-12">
                        <input type="text" name="title" id="title" placeholder="Title" value="{{@old('title')}}" class="form-control h-40 mb-2" required>
                    </div>
                    <div class="col-md-2 col-sm-12">
                        <select name="hakakses" id="hakakses" placeholder="Dapat dilihat oleh..." class="form-control h-40 px-0 mb-2" required>
                            <option value="" selected disabled>Dapat dilihat oleh...</option>
                            <option value="0" {{@old('hakakses') == '0' ? 'selected' : ''}}>Public</option>
                            <option value="1" {{@old('hakakses') == '1' ? 'selected' : ''}}>Private</option>
                        </select>
                    </div>
                    <div class="col-md-12" id="restricted" style="display:none;">
                        <select class="select2 form-control h-40 mb-2" id="user_private" name="user[]" multiple required>
                        </select>
                    </div>
                    <div class="col-md-12 mt-2">
                        ...
                        <div class="tab-content" id="myTabContent">
                            <div class="tab-pane fade show active" id="post-isi" role="tabpanel" aria-labelledby="post-isi">
                                <textarea name="editorpost" id="editorpost" class="form-control" required>{{@old('editorpost')}}</textarea>
                            </div>
                            <div class="tab-pane fade" id="link-isi" role="tabpanel" aria-labelledby="link-isi">
                                <textarea name="editorlink" id="editorlink" class="form-control" placeholder="Contoh: http://example.com atau https://example.com" required>{{@old('editorlink')}}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-md-12 mt-4">
                <div class="d-flex justify-content-end">
                    ...
                    <button class="btn btn-sm btn-outline-primary mx-1" type="button" id="btn-draft">Save Draft</button>
                    <button class="btn btn-sm btn-primary mx-1" type="button" id="btn-submit">Submit</button>
                </div>
            </div>
        </form>
    </div>
