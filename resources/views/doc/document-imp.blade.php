...
@forelse ($document as $doc)
    <tr class="rowdoc">
        <td id="td-attachment" class="d-flex align-items-center" style="font-size: 14px">
            <input type="checkbox" name="file-{{$step}}" class="mr-2 file" onchange='klik(`{{$step}}`);' value="{{$doc->url_file}}">
            <i class="fas fa-file mr-2"></i>
            <a class="btn p-0 text-primary" style="font-size: 14px;text-align:left;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;" href="{{Config::get('app.url').'storage/'.$doc->url_file}}" download="{{$doc->nama}}">{{$doc->nama}}</a>
        </td>
