...
const cekDivisi = (selOrUn, value) => {
  if (
    $("#divisi-lesson-init").hasClass("is-invalid") ||
    $("#divisi-lesson-init").hasClass("is-valid")
  ) {
    if (this.value == "") {
      ...
      if (data.data.length > 0) {
        for (let index = 0; index < data.data.length; index++) {
          if (selOrUn === "select") {
            ...
            $("#divisi-lesson-init").append(
              `<option value='${data.data[index].id}' data-value='${data.data[index].divisi}' selected>${data.data[index].divisi}</option>`
            );
          } else {
            $(
              `#divisi-lesson-init option[value="${data.data[index].id}"]`
            ).detach();
            // div_short.push(data.data[index].shortname)
            //option += `<option value='${data.data[index].id}' data-value='${data.data[index].divisi}'>${data.data[index].divisi}</option>`;
...
function getData(tahap, divisi, search) {
  ...
  if (lastpath === "mylesson") {
    url = `${getCookie(
      "url_be"
    )}api/mylessonlearned/all?tahap=${tahap}&divisi=${divisi}&search=${search}`;
    ...
    url = `${getCookie(
      "url_be"
    )}api/mylessonlearned/all?tahap=${lastpath}&divisi=${divisi}&search=${search}`;
    document.getElementById("btn-sort-lesson").innerHTML = lastpath.split('%20').join(' ');
    document.getElementById("main-title-lesson").innerHTML = lastpath.split('%20').join(' ');
  }
    ...
    success: function (data) {
      ...
      if (data.data.length !== 0) {
        for (let i = 0; i < data.data.length; i++) {
          if (
            data.data[i].lesson_learned.length !== 0 ||
            data.data[i].consultant.length !== 0
          ) {
            for (
              let ilesson = 0;
              ilesson < data.data[i].lesson_learned.length;
              ilesson++
            ) {
            ...
            $("#container-review")
              .append(`<div class="card card-body w-100 d-flex mb-1" style="border-radius: 10px">
                        <div class="row">
                            <div class="col-2">
                                <a href="${
                                  uri + "/katalog/" + data.data[i].divisi.divisi
                                }" class="text-primary">${
              data.data[i].divisi.direktorat
            }</a>
                            </div>
                            <div class="col-3">
                                <a href="${
              ...
              data.data[i].divisi.divisi
            }</a>
                            </div>
                            <div class="col-3">
                                <a href="${
                                  uri + "/project/" + data.data[i].slug
                                }">${data.data[i].nama}</a>
                            </div>
                            <div class="col-2">
                                <a href="${
                                  data.data[i].consultant[0] !== undefined
                                    ? uri +
                                      "/consultant/" +
                                      data.data[i].consultant[0].id
                                    : ''
                                }">${data.data[i].consultant[0] !== undefined ? data.data[i].consultant[0].nama : 'Internal'}</a>
                            </div>
                            <div class="col-2">
                                <a href="${
                                  uri + "/kontribusi/" + data.data[i].slug
                                }" class="btn btn-outline-secondary fas fa-pen"></a>
                                <button class="btn btn-outline-secondary fas fa-caret-down" data-toggle="collapse" href="#${data.data[
                                  i
                                ].nama.replaceAll(' ', '')}" aria-expanded="false" aria-controls="${data.data[
              i
            ].nama.replaceAll(' ', '')}"></button>
                            </div>
                        </div>
                            <div class="collapse" id="${data.data[
                              i
                            ].nama.replaceAll(' ', '')}">
                                <div class="card-body">
                                    <div class="row">
