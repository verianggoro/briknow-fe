...
const getdatalist = () =>{
var url = `${uri}/manageforum/removed/${search}?page=${page}`;
    $.ajax({
        url: url,
        headers: {
            "X-CSRF-TOKEN"  :   csrf,
...
const restore = (id) =>{
    let t = "{{$token_auth}}";
    var url = `${uri}/manageforum/removed/restore/` + id;
    swal.fire({ title: "Yakin ingin restore?", text: "", icon: "warning", showCancelButton: !0, confirmButtonColor: "#3085d6", cancelButtonColor: "#d33", confirmButtonText: "Ya", cancelButtonText: "Tidak" }).then((i) => {
        if(i.isConfirmed){
            $.ajax({
                url: url,
                headers: {
                ...
                success: function () {
                    $('.senddataloader').hide();
                    swal.fire({ title: "Postingan berhasil direstore!", text: "", icon: "success",confirmButtonColor: "#3085d6", confirmButtonText: "Ok"}).then((i) => {
                        if(i.isConfirmed){
                            location.reload();
                        }else{
                            location.reload();
                ...
                error: function () {
                    $('.senddataloader').hide();
                    Swal.fire({ icon: "error", title: "Oops...", text: "Postingan gagal direstore!" });
                },
            })
        }else{
            swal.fire("Postingan tidak jadi direstore!");
        }
    });
};

function sortPrivate() {
var url = `${uri}/manageforum/removed/sort/private/${search}?page=${page}`;
    $.ajax({
        url: url,
        headers: {
            "X-CSRF-TOKEN"  :   csrf,
...
function sortPublic() {
var url = `${uri}/manageforum/removed/sort/public/${search}?page=${page}`;
    $.ajax({
        url: url,
        headers: {
            "X-CSRF-TOKEN"  :   csrf,
